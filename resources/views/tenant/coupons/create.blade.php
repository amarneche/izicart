...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.coupons.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('coupon_type') ? 'is-invalid' : '' }}" name="coupon_type" id="coupon_type" required>
                    ...
                    @foreach(App\Models\Coupon::COUPON_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('coupon_type', 'percentage') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('exipre_at') ? 'is-invalid' : '' }}" type="text" name="exipre_at" id="exipre_at" value="{{ old('exipre_at') }}">
