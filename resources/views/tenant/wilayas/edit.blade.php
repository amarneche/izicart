...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.wilaya.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("tenant.wilayas.update", [$wilaya->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="wilaya">{{ trans('cruds.wilaya.fields.wilaya') }}</label>
                <input class="form-control {{ $errors->has('wilaya') ? 'is-invalid' : '' }}" type="text" name="wilaya" id="wilaya" value="{{ old('wilaya', $wilaya->wilaya) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('wilaya_ar') ? 'is-invalid' : '' }}" type="text" name="wilaya_ar" id="wilaya_ar" value="{{ old('wilaya_ar', $wilaya->wilaya_ar) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_available') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="is_available" id="is_available" value="1" {{ $wilaya->is_available || old('is_available', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('default_cost') ? 'is-invalid' : '' }}" type="text" name="default_cost" id="default_cost" value="{{ old('default_cost', $wilaya->default_cost) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_methods') ? 'is-invalid' : '' }}" name="payment_methods[]" id="payment_methods" multiple>
                    @foreach($payment_methods as $id => $payment_method)
                        <option value="{{ $id }}" {{ (in_array($id, old('payment_methods', [])) || $wilaya->payment_methods->contains($id)) ? 'selected' : '' }}>{{ $payment_method }}</option>
                    @endforeach
                </select>
