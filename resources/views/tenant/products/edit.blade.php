    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="featured_image">{{ trans('cruds.product.fields.featured_image') }}</label>
            ...
            <div class="form-group">
                <label for="stock_quantity">{{ trans('cruds.product.fields.stock_quantity') }}</label>
                <input class="form-control {{ $errors->has('stock_quantity') ? 'is-invalid' : '' }}" type="number" name="stock_quantity" id="stock_quantity" value="{{ old('stock_quantity', $product->stock_quantity) }}" step="1">
                @if($errors->has('stock_quantity'))
                    <div class="invalid-feedback">
                        {{ $errors->first('stock_quantity') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.stock_quantity_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sizes') ? 'is-invalid' : '' }}" name="sizes[]" id="sizes" multiple>
                    @foreach($sizes as $id => $size)
                        <option value="{{ $id }}" {{ (in_array($id, old('sizes', [])) || $product->sizes->contains($id)) ? 'selected' : '' }}>{{ $size }}</option>
                    @endforeach
                </select>
                @if($errors->has('sizes'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <label for="coupons">{{ trans('cruds.product.fields.coupon') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('coupons') ? 'is-invalid' : '' }}" name="coupons[]" id="coupons" multiple>
                    @foreach($coupons as $id => $coupon)
                        <option value="{{ $id }}" {{ (in_array($id, old('coupons', [])) || $product->coupons->contains($id)) ? 'selected' : '' }}>{{ $coupon }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.coupon_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    url: '{{ route('tenant.products.storeMedia') }}',
    maxFilesize: 2, // MB
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('tenant.products.storeCKEditorImages') }}', true);
