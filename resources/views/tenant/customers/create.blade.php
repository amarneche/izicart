...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.customers.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('wilaya') ? 'is-invalid' : '' }}" name="wilaya_id" id="wilaya_id">
                    @foreach($wilayas as $id => $entry)
                        <option value="{{ $id }}" {{ old('wilaya_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('commune') ? 'is-invalid' : '' }}" name="commune_id" id="commune_id">
                    @foreach($communes as $id => $entry)
                        <option value="{{ $id }}" {{ old('commune_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
