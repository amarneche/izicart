...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.menus.update", [$menu->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('placement') ? 'is-invalid' : '' }}" name="placement" id="placement" required>
                    ...
                    @foreach(App\Models\Menu::PLACEMENT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('placement', $menu->placement) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
