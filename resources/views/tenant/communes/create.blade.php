...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.commune.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("tenant.communes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="commune">{{ trans('cruds.commune.fields.commune') }}</label>
                <input class="form-control {{ $errors->has('commune') ? 'is-invalid' : '' }}" type="text" name="commune" id="commune" value="{{ old('commune', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('commune_ar') ? 'is-invalid' : '' }}" type="text" name="commune_ar" id="commune_ar" value="{{ old('commune_ar', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('wilayas') ? 'is-invalid' : '' }}" name="wilayas[]" id="wilayas" multiple required>
                    @foreach($wilayas as $id => $wilaya)
                        <option value="{{ $id }}" {{ in_array($id, old('wilayas', [])) ? 'selected' : '' }}>{{ $wilaya }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_available') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="is_available" id="is_available" value="1" required {{ old('is_available', 0) == 1 || old('is_available') === null ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('delivery_cost') ? 'is-invalid' : '' }}" type="number" name="delivery_cost" id="delivery_cost" value="{{ old('delivery_cost', '0') }}" step="0.01">
