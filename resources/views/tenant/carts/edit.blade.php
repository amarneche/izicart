...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.carts.update", [$cart->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cart_total') ? 'is-invalid' : '' }}" type="number" name="cart_total" id="cart_total" value="{{ old('cart_total', $cart->cart_total) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Cart::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $cart->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
