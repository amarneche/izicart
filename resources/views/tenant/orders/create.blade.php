...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("tenant.orders.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ship_to_wilaya') ? 'is-invalid' : '' }}" name="ship_to_wilaya_id" id="ship_to_wilaya_id" required>
                    @foreach($ship_to_wilayas as $id => $entry)
                        <option value="{{ $id }}" {{ old('ship_to_wilaya_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shipt_to_commune') ? 'is-invalid' : '' }}" name="shipt_to_commune_id" id="shipt_to_commune_id" required>
                    @foreach($shipt_to_communes as $id => $entry)
                        <option value="{{ $id }}" {{ old('shipt_to_commune_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Order::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'new-order') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
