@can('order_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('tenant.order-items.create') }}">
                ...
                <tbody>
                    @foreach($orderItems as $key => $orderItem)
                        <tr data-entry-id="{{ $orderItem->id }}">
                            ...
                            <td>
                                @can('order_item_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('tenant.order-items.show', $orderItem->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('order_item_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('tenant.order-items.edit', $orderItem->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('order_item_delete')
                                    <form action="{{ route('tenant.order-items.destroy', $orderItem->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('order_item_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('tenant.order-items.massDestroy') }}",
