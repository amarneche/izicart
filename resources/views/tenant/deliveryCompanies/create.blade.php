...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.deliveryCompany.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("tenant.delivery-companies.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ship_to_wilayas') ? 'is-invalid' : '' }}" name="ship_to_wilayas[]" id="ship_to_wilayas" multiple>
                    @foreach($ship_to_wilayas as $id => $ship_to_wilaya)
                        <option value="{{ $id }}" {{ in_array($id, old('ship_to_wilayas', [])) ? 'selected' : '' }}>{{ $ship_to_wilaya }}</option>
                    @endforeach
                </select>
